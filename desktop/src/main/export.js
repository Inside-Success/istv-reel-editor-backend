...
async function exportReels({ srcPath, outDir, reels, dialog, cameras, onEvent }) {
  ...
  // dialog): appended after the reel content only in the rendered file, never
  // touching the editor timeline. Omitted entirely unless the user both picked
  // a file AND checked the box. Threaded into the Node engine (mediaEngine.cjs)
  // via the per-reel payload; the backend export path keeps its own copy in
  // export/media.cjs.
  if (dialog.includeEndCredits && dialog.endCreditsPath) {
    globalOptions.endCreditsPath = dialog.endCreditsPath;
  }
