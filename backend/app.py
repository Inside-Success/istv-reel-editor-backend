...
async def transcribe(request: Request) -> dict:
    ...
    progress on the client). Same API surface as the Vercel serverless
    backend's chunked endpoints below — the desktop client always uses those
    instead so it doesn't need to know which host it's talking to.
    """
